...
import {MetaMaskWallet, MsgStoreCodeResponse, SecretNetworkClient, TxResponse, TxResultCode, Wallet} from "../src";
import fs from "fs";
import {Account, checkInstantiateSuccess, getValueFromRawLog, localsecretRestApi, sleep, storeSnip20Ibc} from "./utils";
...
test("MetaMaskWallet", async () => {
    ...
    const code_id = await storeSnip20Ibc(secretjs, accounts[accountNumber].address);
    ...
    const tx = await secretjs.tx.compute.instantiateContract(
        {
            ...
            init_msg: {
                ...
                symbol: "SSCRT",
                decimals: 6,
                initial_balances: [{ address: accounts[accountNumber].address, amount: "1" }],
                prng_seed: "eW8=",
                config: {
