...
export const ABCIParams = {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): ABCIParams {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.vote_extensions_enable_height = longToString(
  ...
  toJSON(message: ABCIParams): unknown {
    const obj: any = {};
    message.vote_extensions_enable_height !== undefined &&
      (obj.vote_extensions_enable_height =
        message.vote_extensions_enable_height);
    return obj;
  },
  ...
  fromPartial(object: DeepPartial<ABCIParams>): ABCIParams {
    const message = createBaseABCIParams();
    message.vote_extensions_enable_height =
      object.vote_extensions_enable_height ?? "0";
    return message;
  },
};
